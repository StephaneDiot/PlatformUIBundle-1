...
YUI.add('ez-date-editview', function (Y) {
    ...
    Y.eZ.DateEditView = Y.Base.create('dateEditView', Y.eZ.FieldEditView, [],  {
        ...
        _initializeCalendarWidget: function (e) {
            ...
            if (this.get('field') && this.get('field').fieldValue && this.get('field').fieldValue.timestamp) {
                date = Y.Date.format(new Date(this.get('field').fieldValue.timestamp * 1000));
            }
            this.set('calendar',new Y.Calendar({
                contentBox: calendarNode,
                showPrevMonth: true,
                showNextMonth: true,
                date: Y.Date.parse(date)
            }));
            calendar = this.get('calendar');
            calendar.selectDates([Y.Date.parse(date)]);
            calendar.render();
        ...
        _isDateValid: function () {
            return ((this._getInputValidity().patternMismatch || Y.Date.parse(this._getInputNode().get('value')) === null) ? true : false);
        },
        ...
        _getFieldValue: function () {
            ...
            if (valueOfTextInput !== null){
                ...
                return {timestamp: Y.Number.parse(valueOfTextInput)};
            }
            return null;
        },
    },{
